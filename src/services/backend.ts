  ...
  try {
    ...
    try {
      ...
      if (isTodayOnly) {
        ...
        rf.set("end_date", effStart);
        fetch(`${localBase}/api/assai/dashboard?${rf.toString()}`).catch(() => {});
        fetch(`${localBase}/api/assai/dashboard?endpoint=force_sync_today&mode=one&concurrency=1&max_pages=16&t=${Date.now()}`).catch(() => {});
      }
    } catch {}
    let resp: Response;
    try {
